...
namespace CronusLang.Compiler.SST.Operators.Arithmetic
{
    ...
    {
        public AddOp(SymbolsScope scope, AST.Operators.Arithmetic.AddOp syntaxNode, SemanticTransformer transformer) : base(scope, syntaxNode, transformer)
        {
        }
        ...
        public override void Analyze(SemanticContext context)
        {
            if (!Type.IsResolved)
            {
                    ...
                    Right.Type.TrackDependency(context, out var rightType))
                {
                    ...
                    {
                        throw new Exception($"Invalid addition operation between {leftType.Symbol.FullPath} and {rightType.Symbol.FullPath}");
                    }
